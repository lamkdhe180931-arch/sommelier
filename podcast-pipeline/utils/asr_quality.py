...
def choose_asr_text(
    *,
    ...
    The guard is intentionally conservative: it only overrides ROVER when the
    chosen text is a known Whisper-style boilerplate/foreign outlier, or when the
    Vietnamese models form a clear consensus on very short audio.
    """
...
def should_skip_sepreformer_pair(
    pair: dict[str, Any],
    *,
    ...
    reasons: list[str] = []
    overlap_duration = max(0.0, float(pair.get("overlap_end", 0.0)) - float(pair.get("overlap_start", 0.0)))
    seg1_duration = _duration(pair.get("seg1", {}))
    seg2_duration = _duration(pair.get("seg2", {}))

    if overlap_duration < min_overlap_seconds:
        reasons.append("overlap_lt_min")
    if min(seg1_duration, seg2_duration) < min_segment_seconds:
        reasons.append("segment_lt_min")
