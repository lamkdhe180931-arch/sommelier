...
def load_phowhisper_model(
    model_name: str = PHOWHISPER_MODEL_NAME,
    device=None,
) -> PhoWhisperTranscriber:
    return PhoWhisperTranscriber.from_pretrained(model_name=model_name, device=device)


def load_chunkformer_model(
    model_name: str = CHUNKFORMER_MODEL_NAME,
    device=None,
) -> ChunkFormerTranscriber:
    return ChunkFormerTranscriber.from_pretrained(model_name=model_name, device=device)
